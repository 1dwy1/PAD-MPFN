...
def prepare_distributed_data(cfg, mode="train"):
            ...
            for line in tqdm(f):
                ...
                for news_ID, label in impressions:
                    if label == '0':
                        ...
                        pos.append(news_ID)
                # 如果当前行无数据.即impressions中无数据,就跳过当前行
                if len(pos) == 0 or len(neg) == 0:
                    continue
                # 对于每个正样本，为他分配4个负样本(候选样本)
                for pos_id in pos:
                    # 候选样本,通过标签为0的数据采样,采样方式按照上面的方法
    ...
    # 验证和测试模式，不进行负采样，只是简单地将原始数据根据 GPU 数量进行划分
    ...
    # 将每个GPU分配到的数据写入新的文件,文件名格式为 behaviors_np4_{0}.tsv
